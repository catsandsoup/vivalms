...
import { BookOpen, Clock, Users, Star, CheckCircle, PlayCircle, Search, ChevronDown, Settings, BarChart3, MessageSquare, User, LogOut } from "lucide-react";
interface Module {
  id: string;
  title: string;
  description: string;
...
export default function Dashboard({
  ...
  const filteredModules = modules.filter(module => selectedCategory === "All" || module.category === selectedCategory).filter(module => searchQuery === "" || module.title.toLowerCase().includes(searchQuery.toLowerCase()) || module.description.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 max-w-md mx-8">
              <div className="relative">
                ...
                <input type="text" placeholder="Search modules..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {/* Admin Menu */}
              <div className="relative">
                <Button variant="outline" size="sm" onClick={() => setShowAdminMenu(!showAdminMenu)} className="text-xs">
                  <Settings size={14} className="mr-1" />
                  Admin
                  <ChevronDown size={14} className="ml-1" />
                </Button>
                {showAdminMenu && <div className="absolute right-0 top-full mt-1 w-48 bg-card border border-border rounded-lg shadow-lg z-50">
                    <div className="p-1">
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredModules.map(module => <Card key={module.id} className="card-viva overflow-hidden group">
              <div className="p-6">
                {/* Header */}
                <div className="flex items-start justify-between mb-4">
                  ...
                  {module.isNew && <Badge className="bg-accent text-accent-foreground">
                      New
                    </Badge>}
                </div>
                ...
                {/* Action Button */}
                <Button onClick={() => onModuleSelect(module.id)} className="w-full btn-viva group-hover:scale-105 transition-transform">
                  {module.progress === 0 ? <>
